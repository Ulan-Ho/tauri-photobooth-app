...
export default function TemplatePage({ onSelectDesign }) {
    ...
    const prevSlide = () => {
        ...
        const prevIndex = (currentIndex > 0) ? currentIndex - 1 : availableCanvases.length - 1;
        switchCanvas(availableCanvases[prevIndex].id);
        setCurrentIndex((prevIndex) => (prevIndex + 1) % availableCanvases.length);
    };

    const nextSlide = () => {
        const currentIndex = availableCanvases.findIndex(t => t.id === currentCanvasId);
        const nextIndex = (currentIndex < availableCanvases.length - 1) ? currentIndex + 1 : 0;
        switchCanvas(availableCanvases[nextIndex].id);
        setCurrentIndex((prevIndex) =>
            prevIndex === 0 ? availableCanvases.length - 1 : prevIndex - 1
        );
    };
...
// import { useNavigate } from 'react-router-dom';
// import '../App.css';
// // import '../admin/owl.carousel.min.js';
// import templateTriangle from '../assets/templateTriangle.png';
// import { usePageNavigation } from '../App.jsx';
// import back_img from '../assets/defaultImage.jpeg';
// import { invoke } from "@tauri-apps/api/tauri"
...
// export default function TemplatePage({ onSelectDesign, onSelectTemplate }) {
//     const [bgImage, setBgImage] = useState(localStorage.getItem("back_2") || url(`${back_img}`));
//     const navigate = useNavigate();
//     const { canvases, switchCanvas, currentCanvasId } = useStore();
//     const templates = canvases.filter((canvas) => canvas.canvasProps.available === true);
//     usePageNavigation();
...
//         const fetchImage = async () => {
//             try {
//                 handleTemplateSelect(templates[currentIndex].id);
//                 const image = await invoke('get_image', { imageName: '2_bg.jpeg' });
//                 const url_image = url(`data:image/jpeg;base64,${image}`);
//                 setBgImage(url_image);
//                 localStorage.setItem("back_2", url_image);
...
//     const handleNext = () => {
//         if (selectedDesign && selectedTemplate) navigate('/print');
...
//                                         <img
//                                             src={templates[(currentIndex - 1 + templates.length) % templates.length].canvasProps.webpData}
//                                             alt={templates[(currentIndex - 1 + templates.length) % templates.length].canvasProps.name}
//                                             className="w-64 h-96 opacity-50 cursor-pointer"
//                                             onClick={prevTemplate}
//                                         />
//                                         <img
//                                             src={templates[currentIndex].canvasProps.webpData}
//                                             alt={templates[currentIndex].canvasProps.name}
//                                             className="w-80 h-112 cursor-pointer border-4 border-blue-500"
//                                         />
//                                         <img
//                                             src={templates[(currentIndex - 1 + templates.length) % templates.length].canvasProps.webpData}
